...
void main() {
  ...
  test('connect builds PKCE authorize redirect and stores verifier/state', () async {
    Uri? redirectedTo;
    ...
    expect(
      redirectedTo!.queryParameters['client_id'],
      config.clientId,
    );
    expect(
      redirectedTo!.queryParameters['redirect_uri'],
      config.redirectUri,
    );
    expect(
      redirectedTo!.queryParameters['code_challenge_method'],
      'S256',
    );
  ...
  test('restoreSession exchanges callback code for token and stores session', () async {
    SharedPreferences.setMockInitialValues(<String, Object>{
    ...
    final service = SpotifyPkceAuthService(
      ...
      currentUri: () => Uri.parse(
        '${config.redirectUri}?code=abc123&state=expected-state',
      ),
      replaceCurrentUrl: (_) {},
      redirectTo: (_) async {},
      httpClient: MockClient((request) async {
        expect(request.url.toString(), '${config.accountsBaseUrl}/api/token');
    ...
    final prefs = await SharedPreferences.getInstance();
    expect(prefs.getString('spotify.auth.refresh_token'), 'real-refresh-token');
